...
class CubeJFrame extends JFrame implements KeyListener, MouseListener {
	Solver solver;
 	int faceSize = 49;
	...
	CubeJFrame() {
		...
		Color[] colors = {Color.black, Color.yellow, Color.green, Color.blue, new Color(255, 128, 0), Color.red, Color.lightGray};
		
		for(int i = 0; i < 6; i++) {
			colorPicker[i] = new JButton();
			colorPicker[i].setBackground(colors[i]);
			colorPicker[i].setForeground(colors[i]);
		...
		colorPickerLabel.setForeground(pickedColor);
		...
		for(int i = 0; i < 6; i++) {
			for(int j = 0; j < 9; j++) {
				...
				face[i][j].setForeground(colors[i]);
		...
		for(int i = 0; i < 6; i++) {
			for(int j = 0; j < 9; j++) {
				...
				face[i][j].addActionListener(event -> {
					...
					((JButton)event.getSource()).setForeground(pickedColor);
					solver.update2();
				});
			}
		}
...
//		String[] faceNames = {"U", "D", "F", "B", "L", "R"};
		String[] faceNames = {"上", "下", "前", "后", "左", "右"};
		for(int i = 0; i < 6; i++) 
			face[i][4].setText(faceNames[i]);

		randomScramble = new JButton("<html><b>打乱</b></html>");
		...
		solve = new JButton("<html><b>破解</b></html>");
		...
		clear = new JButton("<html><b>清空</b></html>");
		...
		fill = new JButton("<html><b>新的</b></html>");
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> new CubeJFrame());
	}
}