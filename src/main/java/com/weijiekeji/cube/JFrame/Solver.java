package com.weijiekeji.cube.JFrame;

import com.weijiekeji.kociemba.twophase.Search;
import com.weijiekeji.kociemba.twophase.Tools;
...
class Solver {
	...
	void initialize() {
		...
		try {
			BufferedReader read = new BufferedReader(new InputStreamReader(new ClassPathResource("cube/F2L.txt").getInputStream()));
			for(int i = 0; i < 41; i++) 
				f2lAlgorithms[i] = read.readLine();

			read = new BufferedReader(new InputStreamReader(new ClassPathResource("cube/OLL.txt").getInputStream()));
			for(int i = 0; i < 57; i++) 
				ollAlgorithms[i] = read.readLine();

			read = new BufferedReader(new InputStreamReader(new ClassPathResource("cube/PLL.txt").getInputStream()));
			for(int i = 0; i < 21; i++) 
				pllAlgorithms[i] = read.readLine();
		} catch(Exception e) {
			message.setForeground(Color.red);
			message.setText("Couldn't load files [F2L.txt,OLL.txt,PLL.txt]. Solution will not work.");
			System.out.println("Couldn't load files [F2L.txt,OLL.txt,PLL.txt]. Solution will not work.");
			loaded = false;
		}
        // chushihua jingtai bianliang
//		String res = Search.solution("UUUUUUUUURRRRRRRRRFFFFFFFFFDDDDDDDDDLLLLLLLLLBBBBBBBBB", 22, 5, false);
//		System.out.println("res=>"+res);
	...
	String getAdvanced() {
		...
		String res = Search.solution(s.toString(), 22, 5, false);
		System.out.println("res=>"+res);

		System.out.println("randomCube=>"+ Tools.randomCube());
		return res;
	}
